...
import (
	...
	"github.com/EXCCoin/exccdata/v8/pubsub"
	...
	"github.com/EXCCoin/exccdata/cmd/exccdata/internal/api"
	"github.com/EXCCoin/exccdata/cmd/exccdata/internal/api/insight"
	"github.com/EXCCoin/exccdata/cmd/exccdata/internal/explorer"
	mw "github.com/EXCCoin/exccdata/cmd/exccdata/internal/middleware"
	notify "github.com/EXCCoin/exccdata/cmd/exccdata/internal/notification"
...
func _main(ctx context.Context) error {
	...
	if cfg.ImportSideChains {
		...
		log.Info("Retrieving side chain blocks from exccd...")
...
func listenAndServeProto(ctx context.Context, wg *sync.WaitGroup, listen, proto string, mux http.Handler) {
	...
	go func() {
		var err error
		if proto == "https" {
			err = server.ListenAndServeTLS("exccdata.cert", "exccdata.key")
		} else {
