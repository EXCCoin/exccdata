...
func (p *ChainMonitor) switchToSideChain() (int32, *chainhash.Hash, error) {
	...
	defer func(start time.Time) {
		log.Infof("exccpg: switchToSideChain completed in %v", time.Since(start))
	}(time.Now())
	...
	for i := range p.sideChain {
		...
		if !found || msgBlock.BlockHash() != p.sideChain[i] {
			log.Debugf("block %v not found in stakedb cache, fetching from exccd", p.sideChain[i])
			// Request MsgBlock from exccd
