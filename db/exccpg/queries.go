...
func retrieveTxHistoryByAmountFlow(db *sql.DB, addr string,
	...
	for rows.Next() {
		...
		items.Time = append(items.Time, blockTime)
		items.Received = append(items.Received, exccutil.Amount(received).ToCoin())
		items.Sent = append(items.Sent, exccutil.Amount(sent).ToCoin())
		...
		items.Net = append(items.Net, exccutil.Amount(received-sent).ToCoin())
	}
	return items, nil
}
...
func retrieveTxHistoryByUnspentAmount(db *sql.DB, addr string,
	...
	for rows.Next() {
		...
		totalAmount += amount
		items.Amount = append(items.Amount, exccutil.Amount(totalAmount).ToCoin())
	}
	return items, nil
}
...
func RetrieveTicketsPriceByHeight(db *sql.DB, val int64) (*dbtypes.ChartsData, error) {
	...
	for rows.Next() {
		...
		priceCoin := exccutil.Amount(price).ToCoin()
...
func retrieveCoinSupply(db *sql.DB) (*dbtypes.ChartsData, error) {
	...
	for rows.Next() {
		...
		sum += exccutil.Amount(value).ToCoin()
