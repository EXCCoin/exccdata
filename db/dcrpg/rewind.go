...
func deleteVoutsForBlock(dbTx SqlExecutor, hash dbtypes.ChainHash) (rowsDeleted int64, err error) {
...
func RetrieveTxsBlocksAboveHeight(ctx context.Context, db *sql.DB, height int64) (heights []int64, hashes []dbtypes.ChainHash, err error) {
	var rows *sql.Rows
...
func RetrieveTxsBestBlockMainchain(ctx context.Context, db *sql.DB) (height int64, hash dbtypes.ChainHash, err error) {
...
func DeleteBlockData(ctx context.Context, db *sql.DB, hash dbtypes.ChainHash, height int64) (res dbtypes.DeletionSummary, err error) {
	...
	case sql.ErrNoRows:
		...
		err = nil
		log.Warnf("Block with hash %v not found in block_chain table.", hash)
	case nil:
