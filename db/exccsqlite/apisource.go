...
func (db *wiredDB) BlockSubsidy(height int64, voters uint16) *exccjson.GetBlockSubsidyResult {
...
func (db *wiredDB) GetExplorerTx(txid string) *explorer.TxInfo {
	...
	for i, vin := range txraw.Vin {
		var addresses []string
		// ValueIn is a temporary fix until amountIn is correct from exccd call
		var ValueIn exccutil.Amount
		if !(vin.IsCoinBase() || (vin.IsStakeBase() && i == 0)) {
			var addrs []string
			...
			ValueIn, _ = exccutil.NewAmount(vin.AmountIn)
		}
...
func IsZeroHashP2PHKAddress(checkAddressString string, params *chaincfg.Params) bool {
	...
	address, err := exccutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
...
func (db *wiredDB) UnconfirmedTxnsForAddress(address string) (*txhelpers.AddressOutpoints, int64, error) {
	...
	mempoolTxns, err := db.client.GetRawMempoolVerbose(exccjson.GRMAll)
	if err != nil {
...
func (db *wiredDB) GetMempool() []explorer.MempoolTx {
	mempooltxs, err := db.client.GetRawMempoolVerbose(exccjson.GRMAll)
	if err != nil {
