...
func mainCore() error {
	...
	sqliteDB, cleanupDB, err := exccsqlite.InitWiredDB(&dbInfo,
		ntfnChans.updateStatusDBHeight, exccdClient, activeChain)
	defer cleanupDB()
	if err != nil {
	...
	for {
		...
		go db.SyncChainDBAsync(pgSyncRes, exccdClient, quit,
		...
		if sqliteRes.Error != nil {
			...
			log.Errorf("exccsqlite.SyncDBAsync failed at height %d.", sqliteRes.Height)
			return sqliteRes.Error
		} else if usePG && pgRes.Error != nil {
			log.Errorf("exccpg.SyncChainDBAsync failed at height %d.", pgRes.Height)
			return pgRes.Error
		}

		// Break loop to continue starting exccdata.
	...
	collector := blockdata.NewCollector(exccdClient, activeChain, sqliteDB.GetStakeDB())
	if collector == nil {
	...
	cerr := registerNodeNtfnHandlers(exccdClient)
	if cerr != nil {
		return fmt.Errorf("RPC client error: %v (%v)", cerr.Error(), cerr.Cause())
	}

	// Start web API
	app := newContext(exccdClient, &sqliteDB, cfg.IndentJSON)
...
func listenAndServeProto(listen, proto string, mux http.Handler) error {
	...
	if proto == "https" {
		go func() {
			errChan <- http.ListenAndServeTLS(listen, "exccdata.cert", "exccdata.key", mux)
		}()
	} else {
		go func() {
